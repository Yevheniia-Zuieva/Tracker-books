...
import { 
  ArrowLeft, Heart, Trash2, Edit2, Clock, Loader2, Save, ChevronDown 
} from 'lucide-react';
...
const AllNotesPage = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const booksWithNotes = (Array.isArray(data) ? data : data.results || [])
  ...
  const handleToggleFavorite = async (id, currentStatus) => {
    try {
      ...
      setBooks(prev => prev.map(b => b.id === id ? { ...b, isFavorite: !currentStatus } : b));
  ...
  const handleUpdateNote = async (id) => {
    try {
      ...
      setBooks(prev => prev.map(b => b.id === id ? { ...b, note: editValue } : b));
      setEditingId(null);
    } catch (err) { console.error(err); }
  };
  ...
  const processedNotes = useMemo(() => {
    ...
    result.sort((a, b) => {
      if (sortOrder === "newest") return new Date(b.updated_at || b.addedDate) - new Date(a.updated_at || a.addedDate);
      if (sortOrder === "oldest") return new Date(a.updated_at || a.addedDate) - new Date(b.updated_at || b.addedDate);
  ...
  if (isLoading) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh]">
      ...
      <p className="text-muted-foreground font-medium">Завантаження архіву думкок...</p>
    </div>
  );
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8 text-left animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Тут зібрані всі ваші думки</p>
          </div>
        </div>
        
        <div className="bg-primary/10 px-6 py-2 rounded-2xl border border-primary/20 flex flex-col items-end">
          <span className="text-[10px] font-bold text-primary uppercase tracking-tighter">Всього в системі</span>
          <span className="text-2xl font-black text-primary leading-none">{processedNotes.length}</span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-muted/20 p-4 rounded-3xl border border-border/50">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <label className="text-[10px] font-black text-muted-foreground uppercase ml-1">Сортувати:</label>
      ...
      {processedNotes.length === 0 ? (
        <div className="text-center py-24 border-2 border-dashed rounded-[3rem] bg-muted/10">
          <p className="text-muted-foreground font-medium italic">Нотаток не знайдено за вашим запитом.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {processedNotes.map((book) => (
            <Card key={book.id} className="group hover:shadow-2xl transition-all duration-500 border-l-4 border-l-primary bg-card flex flex-col overflow-hidden">
              <CardContent className="p-6 space-y-4 flex-1 flex flex-col">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground font-bold uppercase tracking-wider">{book.author}</p>
                </div>

                <div className="flex items-center gap-2 text-[10px] text-muted-foreground font-bold">
                  <Clock className="h-3 w-3" />
                  {new Date(book.updated_at || book.addedDate).toLocaleDateString('uk-UA')}
                </div>
                  ...
                  {editingId === book.id ? (
                    <div className="space-y-3 animate-in zoom-in-95 duration-200">
                      ...
                      <div className="flex justify-end gap-2">
                        <Button size="sm" variant="ghost" onClick={() => setEditingId(null)} className="h-7 text-[10px] uppercase font-bold">Скасувати</Button>
                        <Button size="sm" onClick={() => handleUpdateNote(book.id)} className="h-7 px-3 text-[10px] uppercase font-bold"><Save className="w-3 h-3 mr-1" /> Зберегти</Button>
                      </div>
                    </div>
                  ) : (
                ...
                <div className="flex items-center justify-between pt-3 border-t border-border/50 mt-auto">
                  ...
                  >
                    <Heart className={`h-5 w-5 ${book.isFavorite ? 'fill-current scale-110' : ''}`} />
                  </button>

                  <div className="flex gap-1">
                    <button
                      onClick={() => { setEditingId(book.id); setEditValue(book.note); }}
