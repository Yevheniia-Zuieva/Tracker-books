 ...
 * отриманих результатів, пагінацію та можливість ручного додавання книг до бібліотеки.
 */
...
/**
 * Допоміжний компонент для відображення секції фільтрів.
 * @component
 * @param {Object} props - Властивості компонента.
 * @param {React.ComponentType} props.icon - Lucide іконка для заголовка.
 * @param {string} props.title - Назва секції фільтрів.
 * @param {Array<{id: string, label: string}>} props.items - Список опцій фільтрації.
 * @param {string} props.current - Поточне вибране значення.
 * @param {Function} props.onChange - Функція для зміни значення фільтра.
 * @returns {React.JSX.Element} Секція з кнопками-фільтрами.
 */
...
/**
 * Основний компонент сторінки пошуку.
 * * Реалізує функціональні вимоги:
 * - Пошук книг у зовнішніх базах даних (Google Books).
 * - Додавання знайденого контенту до особистої бібліотеки.
 * - Клієнтське сортування та багаторівнева фільтрація.
 * * @component
 ...
 */
  ...
  /** * Динамічне отримання списку доступних жанрів із поточних результатів пошуку.
   * @type {string[]}
   */
  ...
  /**
   * Обробник ініціації нового пошуку.
   * Очищує попередні результати та скидає індекс пагінації.
   * @async
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Видаляємо дублікати за ID навіть у першій пачці
  ...
  /**
   * Обробник дозавантаження результатів (Pagination).
   * Додає нові унікальні записи до існуючого списку.
   * @async
  ...
  const handleLoadMore = async () => {
    ...
    try {
      ...
      setResults((prev) => {
        // Створення Set з уже існуючих ID для швидкої перевірки
        const existingIds = new Set(prev.map((b) => b.id));
        // Фільтрація нових книг, залишаючи лише ті, яких ще немає
  ...
  /** Скидання всіх застосованих клієнтських фільтрів до початкового стану */
  const resetFilters = () => {
    setSortBy("relevance");
  ...
  /**
   * Комплексна логіка обробки результатів на стороні клієнта.
   * Виконує послідовну фільтрацію (мова, жанр, сторінки, дата) та сортування.
   * Використовує `useMemo` для оптимізації продуктивності при великих списках.
   * @type {Array}
   */
  const processedResults = useMemo(() => {
    let list = [...results];

    // Перевірка, чи є в обраному полі буквально те слово, що ввів користувач
    if (query.trim()) {
      ...
      list = list.filter((b) => {
        ...
        if (filter === "author")
          return b.author?.toLowerCase().includes(lowerQuery);
        if (filter === "genre")
          return b.genre?.toLowerCase().includes(lowerQuery);
        if (filter === "all") {
    ...
    // Фільтрація за мовою (uk, en, others)
    if (selectedLang !== "all") {
      list = list.filter((b) => {
    ...
    // Фільтрація за обсягом (сторінки)
    if (selectedPageRange !== "all") {
      if (selectedPageRange === "short")
        list = list.filter((b) => b.pages <= 150);
      else if (selectedPageRange === "medium")
        list = list.filter((b) => b.pages > 150 && b.pages <= 400);
      else if (selectedPageRange === "long")
        list = list.filter((b) => b.pages > 400);
    }

    // Фільтрація за періодом (десятиліття)
    if (selectedDecade !== "all") {
      const year = parseInt(selectedDecade);
      list = list.filter((b) => b.year >= year && b.year < year + 10);
    }

    // Сортування за вибраним критерієм
    switch (sortBy) {
      case "title-asc":
  ...
  /**
   * Збереження книги з Google Books до особистої бібліотеки користувача.
   * @async
   * @param {Object} book - Об'єкт книги з результатів пошуку.
   */
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 space-y-8 min-h-[500px]">
          {processedResults.length > 0 ? (
            ...
            hasSearched &&
            !isLoading && (
              <div className="text-center py-20 border-2 border-dashed rounded-3xl bg-muted/20 text-muted-foreground">
                Нічого не знайдено
              </div>
            )
          )}
